...
const MainInfos = () => {
    ...
    const petFoods =
        data['Pet-Food'][0][dataHeaderPetFood].map((d, k) => {
            ...
            if (d.product_name.toLowerCase().startsWith(usingContext.searchState.toLowerCase()) || d.unit_price.toString().startsWith(usingContext.searchState)) {
                ...
                return (
                    <Col md={4} key={k} >
                        <Card className="mb-4">
                            <Card.Img height={150} variant="top" src={images.images[0]['Pet-Food'][dataHeaderPetFood][k]} alt="Product" />
                            <Card.Body>
    ...
    const otherFoods = data[dataHeader].map((d, k) => {
        console.log(d);
        if (d.product_name.toLowerCase().startsWith(usingContext.searchState.toLowerCase()) || d.unit_price.toString().startsWith(usingContext.searchState)) {
    ...
    return (
        ...
        <Col md={9} >
            <Row>
                ...
                {VisibilityOfPetFood && petFoods}

                {!VisibilityOfPetFood && otherFoods}
            </Row>
        </Col>
    )
}
